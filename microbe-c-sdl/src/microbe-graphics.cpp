...
MicrobeRenderer::MicrobeRenderer(SDL_Window *window)
{
    TTF_Init();

    this->microbe_window = window;

    this->microbe_vramCache = SDL_CreateRGBSurfaceWithFormat(0, 32 * 8, 32 * 8, 32, SDL_PIXELFORMAT_RGBA8888);
    this->microbe_textCache = SDL_CreateRGBSurfaceWithFormat(0, 20 * 8, 18 * 8, 32, SDL_PIXELFORMAT_RGBA8888);
    this->microbe_framebufferCache = SDL_CreateRGBSurfaceWithFormat(0, 160, 144, 32, SDL_PIXELFORMAT_RGBA8888);
...
MicrobeRenderer::~MicrobeRenderer()
{
    SDL_FreeSurface(microbe_vramCache);
    SDL_FreeSurface(microbe_textCache);
    SDL_FreeSurface(microbe_framebufferCache);
    TTF_CloseFont(this->microbe_font);
    TTF_Quit();
}

void MicrobeRenderer::setTile(int index, byte_t *data)
{
    SDL_LockSurface(microbe_tiles_cache[index]);
    uint32_t *pixels = (uint32_t *)microbe_tiles_cache[index]->pixels;
    for (int i = 0; i < 64; i++)
    {
        microbe_tiles[index][i] = data[i];
...
void MicrobeRenderer::setVram(int x, int y, byte_t tileIndex)
{
    microbe_vram[y][x] = tileIndex;

    microbe_isDirty = true;
}

void MicrobeRenderer::setScroll(int x, int y)
{
    microbe_scrollX = x;
    microbe_scrollY = y;
...
void MicrobeRenderer::setSprite(int index, sprite_t *sprite)
{
    microbe_sprites[index] = *sprite;
}

sprite_t *MicrobeRenderer::getSprite(int index)
{
    return &microbe_sprites[index];
}

void MicrobeRenderer::setPalette(int index, SDL_Color *colors)
{
    microbe_palette[index][1] = colors[0];
    microbe_palette[index][2] = colors[1];
    microbe_palette[index][3] = colors[2];
...
void MicrobeRenderer::setChar(int x, int y, char c)
{
    if (x < 20 && y < 18)
    {
        microbe_textBuffer[(y * 20) + x] = c;
    }
}

void MicrobeRenderer::setString(int x, int y, const char *str)
{
    for (int i = 0; i < strlen(str); i++)
    {
        if (x + i < 20 && y < 18)
        {
            microbe_textBuffer[(y * 20) + x + i] = str[i];
        }
        else
        {
            break;
...
void MicrobeRenderer::UpdateVramCache()
{
...
void MicrobeRenderer::UpdateTextCache()
{
    SDL_FillRect(microbe_textCache, NULL, SDL_MapRGBA(microbe_textCache->format, 0, 0, 0, 255));
